// 埋め込み型レストランデータ
const RESTAURANT_DATA = {
  "restaurants": [
    {
      "id": "crispy_salad",
      "name": "Crispy Salad Works",
      "logo": "https://images.unsplash.com/photo-1512621776951-a57141f2eefd?w=100&h=100&fit=crop",
      "category": "サラダ専門店",
      "description": "新鮮野菜とタンパク質豊富な具材で作るカスタムサラダ",
      "website": "https://crispy-salad.com",
      "established": "2018",
      "locations": ["東京", "大阪", "名古屋"],
      "last_updated": "2024-12-01"
    },
    {
      "id": "subway",
      "name": "サブウェイ",
      "logo": "https://images.unsplash.com/photo-1586190848861-99aa4a171e90?w=100&h=100&fit=crop",
      "category": "サンドイッチチェーン",
      "description": "カスタマイズ可能なサブマリンサンドイッチ",
      "website": "https://subway.co.jp",
      "established": "1965",
      "locations": ["全国"],
      "last_updated": "2024-12-01"
    },
    {
      "id": "yoshinoya",
      "name": "吉野家",
      "logo": "https://images.unsplash.com/photo-1603133872878-684f208fb84b?w=100&h=100&fit=crop",
      "category": "牛丼チェーン",
      "description": "手軽に食べられる牛丼・定食メニュー",
      "website": "https://yoshinoya.com",
      "established": "1899",
      "locations": ["全国"],
      "last_updated": "2024-12-01"
    },
    {
      "id": "mos_burger",
      "name": "モスバーガー",
      "logo": "https://images.unsplash.com/photo-1571091718767-18b5b1457add?w=100&h=100&fit=crop",
      "category": "ハンバーガーチェーン",
      "description": "新鮮野菜を使った手作りハンバーガー",
      "website": "https://mos.jp",
      "established": "1972",
      "locations": ["全国"],
      "last_updated": "2024-12-01"
    },
    {
      "id": "starbucks",
      "name": "スターバックス",
      "logo": "https://images.unsplash.com/photo-1461023058943-07fcbe16d735?w=100&h=100&fit=crop",
      "category": "カフェチェーン",
      "description": "コーヒーとフードメニュー",
      "website": "https://starbucks.co.jp",
      "established": "1971",
      "locations": ["全国"],
      "last_updated": "2024-12-01"
    }
  ],
  "menus": [
    {
      "id": "crispy_protein_caesar",
      "restaurant_id": "crispy_salad",
      "name": "プロテインシーザーサラダ（チキン）",
      "category": "サラダ",
      "description": "グリルチキンとロメインレタスのシーザーサラダ",
      "image": "https://images.unsplash.com/photo-1512621776951-a57141f2eefd?w=300&h=200&fit=crop",
      "price": 980,
      "size": "レギュラー",
      "nutrition": {
        "calories": 285,
        "protein": 32.5,
        "carbs": 8.2,
        "fat": 12.8,
        "fiber": 3.5,
        "sodium": 850,
        "sugar": 2.1,
        "saturated_fat": 3.2,
        "trans_fat": 0.0,
        "cholesterol": 85,
        "calcium": 180,
        "iron": 2.8,
        "potassium": 420,
        "vitamin_a": 850,
        "vitamin_c": 25,
        "vitamin_d": 0,
        "vitamin_e": 8.5,
        "vitamin_k": 15,
        "thiamin": 0.12,
        "riboflavin": 0.25,
        "niacin": 12.5,
        "vitamin_b6": 0.85,
        "folate": 45,
        "vitamin_b12": 2.8,
        "phosphorus": 280,
        "magnesium": 85,
        "zinc": 3.2,
        "selenium": 28
      },
      "allergens": ["乳", "卵", "小麦"],
      "is_vegetarian": false,
      "is_vegan": false,
      "is_gluten_free": false,
      "available": true,
      "seasonal": false,
      "added_date": "2024-11-01",
      "last_updated": "2024-12-01"
    },
    {
      "id": "crispy_quinoa_bowl",
      "restaurant_id": "crispy_salad",
      "name": "キヌアボウル（サーモン）",
      "category": "ボウル",
      "description": "キヌアベースにグリルサーモンと季節野菜",
      "image": "https://images.unsplash.com/photo-1546793665-c74683f339c1?w=300&h=200&fit=crop",
      "price": 1280,
      "size": "レギュラー",
      "nutrition": {
        "calories": 425,
        "protein": 28.5,
        "carbs": 35.2,
        "fat": 18.5,
        "fiber": 6.2,
        "sodium": 720,
        "sugar": 4.8,
        "saturated_fat": 4.2,
        "trans_fat": 0.0,
        "cholesterol": 65,
        "calcium": 120,
        "iron": 3.5,
        "potassium": 580,
        "vitamin_a": 1200,
        "vitamin_c": 15,
        "vitamin_d": 18,
        "vitamin_e": 12.5,
        "vitamin_k": 8,
        "thiamin": 0.18,
        "riboflavin": 0.22,
        "niacin": 8.5,
        "vitamin_b6": 0.65,
        "folate": 85,
        "vitamin_b12": 4.2,
        "phosphorus": 320,
        "magnesium": 95,
        "zinc": 2.8,
        "selenium": 45
      },
      "allergens": ["魚"],
      "is_vegetarian": false,
      "is_vegan": false,
      "is_gluten_free": true,
      "available": true,
      "seasonal": false,
      "added_date": "2024-10-15",
      "last_updated": "2024-12-01"
    },
    {
      "id": "subway_turkey_breast",
      "restaurant_id": "subway",
      "name": "ターキーブレスト（15cm）",
      "category": "サンドイッチ",
      "description": "低脂肪ターキーブレストのサンドイッチ",
      "image": "https://images.unsplash.com/photo-1586190848861-99aa4a171e90?w=300&h=200&fit=crop",
      "price": 450,
      "size": "15cm",
      "nutrition": {
        "calories": 272,
        "protein": 18.0,
        "carbs": 46.0,
        "fat": 3.5,
        "fiber": 5.0,
        "sodium": 1260,
        "sugar": 6.2,
        "saturated_fat": 1.2,
        "trans_fat": 0.0,
        "cholesterol": 35,
        "calcium": 95,
        "iron": 2.8,
        "potassium": 340,
        "vitamin_a": 450,
        "vitamin_c": 12,
        "vitamin_d": 2.5,
        "vitamin_e": 3.2,
        "vitamin_k": 8,
        "thiamin": 0.15,
        "riboflavin": 0.18,
        "niacin": 8.5,
        "vitamin_b6": 0.45,
        "folate": 35,
        "vitamin_b12": 1.8,
        "phosphorus": 180,
        "magnesium": 42,
        "zinc": 1.8,
        "selenium": 15
      },
      "allergens": ["小麦", "乳"],
      "is_vegetarian": false,
      "is_vegan": false,
      "is_gluten_free": false,
      "available": true,
      "seasonal": false,
      "added_date": "2024-01-01",
      "last_updated": "2024-12-01"
    },
    {
      "id": "subway_veggie_delite",
      "restaurant_id": "subway",
      "name": "ベジーデライト（15cm）",
      "category": "サンドイッチ",
      "description": "新鮮野菜たっぷりのベジタリアンサンドイッチ",
      "image": "https://images.unsplash.com/photo-1592415486689-125cbbfcbee2?w=300&h=200&fit=crop",
      "price": 390,
      "size": "15cm",
      "nutrition": {
        "calories": 230,
        "protein": 8.0,
        "carbs": 44.0,
        "fat": 2.5,
        "fiber": 5.0,
        "sodium": 410,
        "sugar": 8.5,
        "saturated_fat": 0.8,
        "trans_fat": 0.0,
        "cholesterol": 0,
        "calcium": 85,
        "iron": 2.2,
        "potassium": 280,
        "vitamin_a": 750,
        "vitamin_c": 18,
        "vitamin_d": 0,
        "vitamin_e": 2.8,
        "vitamin_k": 25,
        "thiamin": 0.12,
        "riboflavin": 0.15,
        "niacin": 4.2,
        "vitamin_b6": 0.35,
        "folate": 45,
        "vitamin_b12": 0.0,
        "phosphorus": 120,
        "magnesium": 38,
        "zinc": 1.2,
        "selenium": 8
      },
      "allergens": ["小麦"],
      "is_vegetarian": true,
      "is_vegan": false,
      "is_gluten_free": false,
      "available": true,
      "seasonal": false,
      "added_date": "2024-01-01",
      "last_updated": "2024-12-01"
    },
    {
      "id": "yoshinoya_gyudon_regular",
      "restaurant_id": "yoshinoya",
      "name": "牛丼（並盛）",
      "category": "丼",
      "description": "定番の牛丼並盛",
      "image": "https://images.unsplash.com/photo-1603133872878-684f208fb84b?w=300&h=200&fit=crop",
      "price": 426,
      "size": "並盛",
      "nutrition": {
        "calories": 652,
        "protein": 19.9,
        "carbs": 92.2,
        "fat": 18.1,
        "fiber": 1.0,
        "sodium": 1180,
        "sugar": 12.5,
        "saturated_fat": 6.8,
        "trans_fat": 0.2,
        "cholesterol": 55,
        "calcium": 45,
        "iron": 2.5,
        "potassium": 420,
        "vitamin_a": 180,
        "vitamin_c": 5,
        "vitamin_d": 0.8,
        "vitamin_e": 1.8,
        "vitamin_k": 3,
        "thiamin": 0.08,
        "riboflavin": 0.12,
        "niacin": 6.5,
        "vitamin_b6": 0.28,
        "folate": 18,
        "vitamin_b12": 1.2,
        "phosphorus": 165,
        "magnesium": 35,
        "zinc": 3.8,
        "selenium": 12
      },
      "allergens": ["小麦", "大豆"],
      "is_vegetarian": false,
      "is_vegan": false,
      "is_gluten_free": false,
      "available": true,
      "seasonal": false,
      "added_date": "2024-01-01",
      "last_updated": "2024-12-01"
    },
    {
      "id": "yoshinoya_chicken_salad",
      "restaurant_id": "yoshinoya",
      "name": "チキンサラダ",
      "category": "サラダ",
      "description": "グリルチキンと新鮮野菜のサラダ",
      "image": "https://images.unsplash.com/photo-1540420773420-3366772f4999?w=300&h=200&fit=crop",
      "price": 390,
      "size": "レギュラー",
      "nutrition": {
        "calories": 185,
        "protein": 22.5,
        "carbs": 8.5,
        "fat": 7.2,
        "fiber": 3.8,
        "sodium": 650,
        "sugar": 4.2,
        "saturated_fat": 2.8,
        "trans_fat": 0.0,
        "cholesterol": 75,
        "calcium": 125,
        "iron": 2.2,
        "potassium": 380,
        "vitamin_a": 950,
        "vitamin_c": 22,
        "vitamin_d": 0,
        "vitamin_e": 5.5,
        "vitamin_k": 18,
        "thiamin": 0.15,
        "riboflavin": 0.28,
        "niacin": 10.5,
        "vitamin_b6": 0.65,
        "folate": 42,
        "vitamin_b12": 2.5,
        "phosphorus": 225,
        "magnesium": 68,
        "zinc": 2.8,
        "selenium": 25
      },
      "allergens": ["卵", "乳"],
      "is_vegetarian": false,
      "is_vegan": false,
      "is_gluten_free": true,
      "available": true,
      "seasonal": false,
      "added_date": "2024-03-01",
      "last_updated": "2024-12-01"
    },
    {
      "id": "mos_green_burger",
      "restaurant_id": "mos_burger",
      "name": "グリーンバーガー",
      "category": "ハンバーガー",
      "description": "レタス包みの低糖質ハンバーガー",
      "image": "https://images.unsplash.com/photo-1571091718767-18b5b1457add?w=300&h=200&fit=crop",
      "price": 420,
      "size": "レギュラー",
      "nutrition": {
        "calories": 238,
        "protein": 15.8,
        "carbs": 6.2,
        "fat": 17.5,
        "fiber": 2.8,
        "sodium": 580,
        "sugar": 3.5,
        "saturated_fat": 5.2,
        "trans_fat": 0.1,
        "cholesterol": 65,
        "calcium": 85,
        "iron": 1.8,
        "potassium": 320,
        "vitamin_a": 380,
        "vitamin_c": 8,
        "vitamin_d": 1.2,
        "vitamin_e": 2.8,
        "vitamin_k": 12,
        "thiamin": 0.12,
        "riboflavin": 0.18,
        "niacin": 8.2,
        "vitamin_b6": 0.35,
        "folate": 25,
        "vitamin_b12": 1.8,
        "phosphorus": 185,
        "magnesium": 45,
        "zinc": 2.5,
        "selenium": 18
      },
      "allergens": ["卵", "乳"],
      "is_vegetarian": false,
      "is_vegan": false,
      "is_gluten_free": true,
      "available": true,
      "seasonal": false,
      "added_date": "2024-06-01",
      "last_updated": "2024-12-01"
    },
    {
      "id": "starbucks_protein_box",
      "restaurant_id": "starbucks",
      "name": "プロテインボックス",
      "category": "軽食",
      "description": "ゆで卵とナッツ、チーズのプロテインボックス",
      "image": "https://images.unsplash.com/photo-1461023058943-07fcbe16d735?w=300&h=200&fit=crop",
      "price": 580,
      "size": "1個",
      "nutrition": {
        "calories": 290,
        "protein": 19.0,
        "carbs": 8.0,
        "fat": 20.5,
        "fiber": 3.0,
        "sodium": 420,
        "sugar": 2.8,
        "saturated_fat": 8.5,
        "trans_fat": 0.0,
        "cholesterol": 185,
        "calcium": 220,
        "iron": 1.5,
        "potassium": 285,
        "vitamin_a": 420,
        "vitamin_c": 2,
        "vitamin_d": 2.8,
        "vitamin_e": 6.5,
        "vitamin_k": 5,
        "thiamin": 0.08,
        "riboflavin": 0.35,
        "niacin": 3.2,
        "vitamin_b6": 0.18,
        "folate": 32,
        "vitamin_b12": 1.2,
        "phosphorus": 285,
        "magnesium": 55,
        "zinc": 2.2,
        "selenium": 22
      },
      "allergens": ["卵", "乳", "ナッツ"],
      "is_vegetarian": true,
      "is_vegan": false,
      "is_gluten_free": true,
      "available": true,
      "seasonal": false,
      "added_date": "2024-09-01",
      "last_updated": "2024-12-01"
    }
  ]
};